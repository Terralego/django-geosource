...
class GeostoreCallBacksTestCase(TestCase):
    ...
    def test_feature_callback(self):
        ...
        feature = geostore_callbacks.feature_callback(source, layer, "id", GEOSGeometry("POINT (0 0)",
                                                                                        srid=3857), {})
    ...
    def test_feature_callback_fail(self):
        ...
        layer = Layer.objects.create(name="test")
        with mock.patch('django_geosource.geostore_callbacks.logger.warning', side_effect=side_effect):
            with self.assertRaisesRegexp(ValueError,
                                         "One record was ignored from source, because of "
                                         "invalid geometry: {'property': 'Hola'}"):
                geostore_callbacks.feature_callback(source, layer, "id", "Not a Point", {"property": "Hola"})
